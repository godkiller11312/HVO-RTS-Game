...
{
    public static Vector2 InputPosition => Input.touchCount > 0 ? (Vector2)Input.GetTouch(0).position : (Vector2)Input.mousePosition;
    ...
    public static bool TryGetShortLeftClickPosition(out Vector2 inputPosition, float maxDistance = 5f)
    {
        ...
        {
            if (Vector2.Distance(m_InitialTouchPosition, HvoUtils.InputPosition) < maxDistance)
            {
                return true;
            }
